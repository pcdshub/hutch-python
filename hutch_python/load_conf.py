...
def load_conf(conf, hutch_dir=None, args=None):
    ...
    try:
        # This is a dictionary of devices that are generated by happi search
        # terms. These devices will be loaded.
        additional_devices = conf['additional_devices']
        if not isinstance(additional_devices, dict):
            logger.error(
                'Invalid additional_devices conf, must have key-value pairs.')
    except KeyError:
        additional_devices = {}
        logger.info(
